...
def graph_noise_comparison(initial_conditons_seed, number_of_tests, modelA, modelB, noiseA, noiseB):
    ...
    config.noise_seed = 0
    config.noise = noiseA
    results_with_noise = modelA(config).run().flatten()

    # Run model with noise
    config.noise_seed = 0 + number_of_tests
    config.noise = noiseB
    results_low_noise = modelB(config).run().flatten()
