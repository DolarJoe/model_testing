import numpy as np
import scipy.stats
from neurolib_driver_wrapper import NeurolibDriverWrapper
from suphopf_config import SupHopfConfig
from tvb_model import TvbModel


def run_test(config):
    resultA = TvbModel(config).run()
    resultB = NeurolibDriverWrapper(config).run()
...
def noise_test(initial_conditons_seed, number_of_tests):
    ...
    for dt in [0.001, 0.01, 0.1]:
        ...
        config.noise_seed = 0
        config.noise = 10.0
        resultsA = NeurolibDriverWrapper(config).run().flatten()
        config.noise_seed = 0 + number_of_tests
        resultsB = TvbModel(config).run().flatten()
